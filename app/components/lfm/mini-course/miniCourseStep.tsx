...
function MiniCourseStep(props: Props) {
  ...
  return (
    <div
      data-testid={`${stepId}-container`}
      className={classNames(isStep2 ? 'laptop:mt-28 ' : '', 'relative items-center miniCourse-vids__courseVideo et-grid-basic col-span-full gap-y-7 tablet:mt-8 laptop:mt-20')}>

      <div
        data-testid={`${stepId}-img-container`}
        className={classNames(imgCss, 'miniCourse-vids__courseVideo--img min-h-[240px] z-2 relative col-start-2 col-span-2 tablet:min-h-[214.59px] laptop:min-h-[316px] desktop:min-h-[377.31px]')}>
        ...
        <div
          data-testid={`${stepId}-bg`}
          className={classNames(bgClass, 'miniCourse-vids__courseVideo--imgBg bg-lfm-pink-200 absolute z-1 w-[225px] laptop:top-[40px]')} />

      </div>

      <div
        data-testid={`${stepId}-content`}
        className={classNames(descClass, 'relative col-span-2 col-start-2 row-start-2 mt-4 mb-8 miniCourse-vids__courseVideo--desc z-3 tablet:mx-2')}>
