      ...
      <div onClick={handleNavigate} className='bg-white shadow-xs rounded-xl hover:cursor-pointer tablet:shadow-none tablet:rounded-none tablet:bg-transparent'>
        <div className="flex flex-row items-center p-5 tablet:flex-col">
          ...
          <div className='w-full max-w-[78px] rounded-lg overflow-hidden shadow-lg mr-4 tablet:mx-auto tablet:max-w-none tablet:mb-4 relative'>
            ...
            {!cookieUnlock && <div className='absolute flex items-center justify-center p-2 -translate-x-1/2 -translate-y-1/2 rounded-lg top-1/2 left-1/2 bg-sage-200 z-3'>
            ...
            {/* locked background */}
            {!cookieUnlock && <div className='absolute top-0 left-0 w-full h-full bg-sage-700 opacity-60 z-2' />}

            {/* LAZYIMAGE */}
            <div className={classNames(!cookieUnlock ? 'opacity-50' : '', 'relative z-1')}>
              <LazyImgix
                id={`Video-${index + 1}`}
                image={video.image}
              />
